...
//полезна€ матрица - матрица без ее полезного размера в первой строке

int** new(int string, int columns)//выделение пам€ти под матрицу
{
	...
	//перва€ строка в матрице будет заполнена ее полезным размером
	//далее в функци€х учитываетс€ что строки матрицы начинаютс€ с индекса 1, а не 0
	matrix[0][0] = string + 1;
	matrix[0][1] = columns;

	//выдел€ем в конце дополнительную строку, т.к. перва€ зан€та размером
...
int** delete(int** matrix)//очистка пам€ти, выделенной под матрицу
{
...
int** multiplication(int** matrix_1, int** matrix_2)//умножение матриц
{
	...
	int** rezult = new(string_1, columns_2);//массив дл€ результата умножени€
...
int size(int size_matrix[4])
{
	system("cls");
	printf("Task 1\n");
	printf("Error. ѕеремножение матриц невозможно. ѕопробуйте еще раз\n");

	for (int i = 0; i < 4; i += 2)
	{
		printf("¬ведите размер матрицы %d\n:", i + 1);
...
void Matrix()
{
	...
	int size_matrix[4];//массив, куда пользователь вводит размеры матриц. подразумеваетс€ что матрицы будет 2
	for (int i = 0; i < 4; i += 2)
	{
		printf("¬ведите размер матрицы %d\n:", i + 1);
		scanf_s("%d", &size_matrix[i]);
